...
export const LATEST_BESTIUM: string =
  bestiumTags[0]?.name.substring(1) ?? VERSION_PLACEHOLDER;

const userdevTags = await fetchGitHubVersions("PaperMC", "paperweight");
export const LATEST_USERDEV: string =
  userdevTags[0]?.name.substring(1) ?? VERSION_PLACEHOLDER;
...
async function fetchPaperVersion(): Promise<string> {
  ...
  try {
    response = await fetch(
      `https://raw.githubusercontent.com/huzvanec/Bestium/${treeSha}/gradle/libs.versions.toml`,
    );
  } catch (e) {
    response = { ok: false } as Response;
  }

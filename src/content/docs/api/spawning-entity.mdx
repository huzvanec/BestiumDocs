...
<Tabs syncKey="lang">
  <TabItem label="Kotlin">
    ```kotlin Bestium.getEntityManager().spawn(Location, Class,
    EntitySpawnReason, CreatureSpawnEvent.SpawnReason, [Consumer]) ```
  </TabItem>
  <TabItem label="Java">
    ```java Bestium.getEntityManager().spawn(Location, Class, EntitySpawnReason,
    CreatureSpawnEvent.SpawnReason, [Consumer]); ```
  </TabItem>
</Tabs>
...
:::caution[Entities with custom names]
At the time of writing this, BetterModel does not support entities with custom names.
:::

:::caution[Attributes must be applied later]
Attributes are applied to entities right before they are spawned, that means
that any attributes applied in this consumer will probably get overwritten by the
